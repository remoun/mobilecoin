...
mod tests {
    ...
    #[test]
    // mc_blockchain_types::BlockSignature --> blockchain::BlockSignature
    fn test_block_signature_from() {
        let mut rng: StdRng = SeedableRng::from_seed([1u8; 32]);

        let source_block_signature = mc_blockchain_types::BlockSignature::new(
    ...
    #[test]
    // blockchain::BlockSignature -> mc_blockchain_types::BlockSignature
    fn test_block_signature_try_from() {
        let mut rng: StdRng = SeedableRng::from_seed([1u8; 32]);

        let expected_block_signature = mc_blockchain_types::BlockSignature::new(
            Ed25519Signature::new([1; 64]),
            (&Ed25519Private::from_random(&mut rng)).into(),
            31337,
        );
        ...
        let block_signature =
            mc_blockchain_types::BlockSignature::try_from(&source_block_signature).unwrap();
    ...
    fn test_blockchain_block_signature_matches_prost() {
        ...
        // Encode using `protobuf`, decode using `prost`.
        {
            ...
            let block_signature_from_prost: mc_blockchain_types::BlockSignature =
