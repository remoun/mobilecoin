...
impl<ID: GenericNodeId + Send + Clone> SCPValidationState<ID> {
    ...
    pub fn new(local_node_id: ID, local_quorum_set: QuorumSet<ID>, logger: Logger) -> Self {
        Self {
            local_node_id,
            local_quorum_set,
            slots_to_externalized_blocks: HashMap::default(),
    ...
    fn find_quorum<'a>(
        ...
        let mut ballot_map: HashMap<&'a BlockID, Vec<&'a QuorumSetMember<ID>>> = HashMap::default();
...
impl<US: BlockStream + 'static, ID: GenericNodeId + Send> BlockStream for SCPValidator<US, ID> {
    type Stream<'s>    = impl Stream<Item = StreamResult<BlockData>> + 's where
    ID: 's;
